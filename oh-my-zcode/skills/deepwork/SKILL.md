...
- create and maintain a local markdown progress file under `.oh-my-zcode/deepwork/`;
- save code/doc deliverables to project paths (e.g. `src/`, `docs/`); reserve
  `.oh-my-zcode/deepwork/` strictly for progress files;
...
```text
.oh-my-zcode/deepwork/<short-task-slug>.md
```
...
These rules keep deepwork state git-local while allowing ZCode to read it.
...
- before starting each phase, replace the ZCode todo list with actionable
...
- when the phase changes module boundaries, dependency direction, or file
  placement, run an `Explore 子智能体` structure scan in parallel with the Oracle gate;
